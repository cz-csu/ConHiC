    ...
    for root, dirs, files in os.walk(groups):
        ...
        #for group in groups:
            ...
            with open(group) as f:
                for line in f:
                    if not line.strip() or line.startswith('#') or line.startswith('scaffold') or line.startswith('Contig') or line.startswith('tig') or re.match(r'^\d', line): 
                        continue
                    nctgs += 1
...
def main():
    ...
    print("average len: ", total_ctg_len/len(fa_len_dict))
    import statistics
    # 计算均值
    mean_ctg_len = statistics.mean(fa_len_dict.values())

    # 计算标准差
    std_ctg_len = statistics.stdev(fa_len_dict.values())
    print(f"Mean Length: {mean_ctg_len}")
    print(f"Standard Deviation: {std_ctg_len}")
    ...
    print('Adjusted contiguity\t{}%'.format(contiguity*anchored_len/total_ctg_len))
