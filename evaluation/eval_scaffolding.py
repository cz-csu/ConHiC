...
import glob
import re
def parse_truth_all_groups(directory_path):
    """
    处理指定目录下的所有group文件
    
    Args:
        directory_path: 包含group文件的目录路径
    
    Returns:
        all_truth_dict: 包含所有group信息的字典
        all_ctg_info_dict: 包含所有contig信息的字典
    """
    all_truth_dict = {}
    all_ctg_info_dict = {}
    
    # 获取所有group文件
    files = glob.glob(os.path.join(directory_path, "group_*.txt"))
    
    for file in files:
        # 从文件名中提取ID，如 group_chr10_1.txt -> chr10_1
        filename = os.path.basename(file)
        if filename.startswith('group_') and filename.endswith('.txt'):
            ID = filename[6:-4]  # 移除"group_"和".txt"
        else:
            ID = os.path.splitext(filename)[0]  # 备用方案
        ...
        with open(file) as f:
            for line in f:
                ...
                ctg = parts[0]
                # 解析ctg字段获取信息
                ctg_parts = ctg.split('_')
                if len(ctg_parts) >= 6:
                    index = int(ctg_parts[2])  # 第三个字段作为index
                    orient = ctg_parts[5]  # 第六个字段作为orient
                    
                    all_truth_dict[ID].append((ctg, orient))
                    all_ctg_info_dict[ctg] = [ID, index, orient]
    
    return all_truth_dict, all_ctg_info_dict
...
def parse_agp(agp, ctg_info_dict):
    ...
    with open(agp) as f:
        for line in f:
            ...
            # statistics for ctgs
            ctg = cols[5]
            #if ctg.startswith('scaffold'):
            #    continue
            ctg_len = int(cols[2]) - int(cols[1]) + 1
            if ctg not in ctg_info_dict:
                #if cols[0].startswith('group'):
                #    ctg_info_dict[ctg] = [cols[0], int(ctg.split('_')[2]), ctg.split('_')[5]]
                #else:
                if ctg.startswith('Contig'):
                    if len(ctg.split('_'))<=6:
                        ctg_info_dict[ctg] = ['unanchored', int(ctg.split('_')[1]), ctg.split('_')[4]]
                    else:
                        ctg_info_dict[ctg] = ['unanchored', int(ctg.split('_')[8]), ctg.split('_')[11]]
                elif ctg.startswith('tig'):
                    if len(ctg.split('_'))<=6:
                       ctg_info_dict[ctg] = ['unanchored', int(ctg.split('_')[1]), ctg.split('_')[4]]
                    else:
                        ctg_info_dict[ctg] = ['unanchored', int(ctg.split('_')[3]), ctg.split('_')[6]]
                elif re.match(r'^\d', ctg):
                    ctg_info_dict[ctg] = ['unanchored', int(ctg.split('_')[1]), ctg.split('_')[4]]
                else:
                    ctg_info_dict[ctg] = ['unanchored', int(ctg.split('_')[2]), ctg.split('_')[5]]
            #assert len(ctg_info_dict[ctg]) == 3
            ctg_info_dict[ctg].append(ctg_len)
            if ctg=="chr1_1_183_26921079_27231415_-_310337:135501-310337":
                print(ctg_info_dict[ctg])
...
def evaluate(scaffold_dict, scaffold_nctgs_dict, truth_dict, ctg_info_dict):
    ...
    def find_lis(ctg_order_list, forward=True):
        ...
        for ctg, order in ctg_order_list:
            ...
            if order in order_ctg_dict:
                print(order,order_ctg_dict[order],ctg)
...
def main():
    ...
    ctg="Contig04045___fragment_2___debris_1_1_25000_-_25000"
    print(ctg.split('_'))
    #truth_dict, ctg_info_dict = parse_truth(args.truth)
    truth_dict, ctg_info_dict = parse_truth_all_groups(args.truth)
    #print(truth_dict)
    scaffold_dict, scaffold_nctgs_dict = parse_agp(args.agp, ctg_info_dict)
    #print(scaffold_dict["chr8_4_7_11571592_14665801_-_3094210"])
